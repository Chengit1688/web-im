<template>
  ...
  <dialogForm ref="dialogFormDom" v-show="showFrom" :formModel="formModel" @handleShowModal="showAddModal" />
</template>
  ...
  return [
    {
      ...
      onClick: () => {
        ...
        dict.edit = true
        showFrom.value = true;
        dialogFormDom.value.setNewsInfo(dict);
      },
...
const columns = [
  ...
  },
  {
    ...
    render: (row) =>
      row.updated_at
        ? formatToDateTime(new Date(row.updated_at * 1000), 'yyyy-MM-dd HH:mm:ss')
        : '-',
    width: 200,
  },
];
...
const showAddModal = (flag) => {
  showFrom.value = !!flag;
  // dialogFormDom.value.setNewsInfo({});
  if (showFrom.value) {
    nextTick(() => {
      dialogFormDom.value.setNewsInfo({});
    });
