...
export function isRootRouter(item) {
  return (
    ...
    item?.children?.filter((item) => !Boolean(item?.meta?.hidden))?.length === 1 // 可以为1
  );
}
...
export function downloadFile(path, name) {
  if (!isSupportDownload) {
    alert('当前浏览器不支持 a 的download属性。请使用Chrome 14+或Firefox 20+');
    return;
  }

  if (!isSupportBlob) {
    alert('当前浏览器不支持 Blob。请使用Chrome 20+或Firefox 13+');
    return;
  }
...
export const formatUrl = (url) => {
  let domain = null;
  if (!url) return url;
  if (window.location.href.indexOf('buzz') !== -1) {
    domain = import.meta.env.VITE_SOURCE_DOMAIN_TEST;
  } else {
    domain = import.meta.env.VITE_SOURCE_DOMAIN;
  }
